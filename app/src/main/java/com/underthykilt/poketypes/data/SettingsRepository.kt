...
private val KEY_SPRITE_GENERATION  = stringPreferencesKey("sprite_generation")
...
class SettingsRepository(private val context: Context) {
    val settings: Flow<AppSettings> = context.settingsDataStore.data.map { prefs ->
        AppSettings(
            ...
            spriteGeneration  = prefs[KEY_SPRITE_GENERATION]?.let { runCatching { SpriteGeneration.valueOf(it)  }.getOrNull() } ?: SpriteGeneration.ALL,
        )
    }
    ...
    suspend fun setSpriteGeneration(gen: SpriteGeneration)           { context.settingsDataStore.edit { it[KEY_SPRITE_GENERATION] = gen.name   } }
}
